...
namespace IdleClinic.Presentation
{
    ...
    {
        private readonly GameObject[] bayMarks=new GameObject[6],futureBays=new GameObject[6],tierDetails=new GameObject[3];
        private readonly Vehicle[] vehicles=new Vehicle[6];private readonly Transform entryArm,exitArm;
        internal ClinicParkingPresentation(ClinicArt art,Transform parent)
        {
            var lot=art.Group("Clinic car park",parent);
            art.Box("Car park surface",lot,new Vector3(-10.325f,-.02f,0),new Vector3(8.35f,.25f,10.2f),"Asphalt");
            art.Box("Parking drive aisle",lot,new Vector3(-10.6f,.108f,0),new Vector3(3.0f,.013f,10.1f),"Ink");
            art.Box("Parking pedestrian path",lot,new Vector3(-6.65f,.105f,-.5f),new Vector3(.85f,.07f,11.1f),"TilePeach");
            art.Box("Parking entrance apron",lot,new Vector3(-10.2f,.04f,-6.25f),new Vector3(5.65f,.12f,2.50f),"Asphalt");
            art.Box("Parking west kerb",lot,new Vector3(-14.5f,.17f,0),new Vector3(.16f,.22f,10.2f),"Clay");
            art.Box("Parking north kerb",lot,new Vector3(-10.3f,.17f,5.07f),new Vector3(8.4f,.22f,.16f),"Clay");
            ...
            for(int i=0;i<6;i++)
            {
                ...
                var car=ClinicStreetLife.Car(art,lot,"Parked patient car "+i,center,i);vehicles[i]=new Vehicle(car, i);
        ...
        internal void Render(ClinicState state,int level,bool reducedMotion)
        {
            ...
            for(int i=0;i<3;i++)tierDetails[i].SetActive(i<level);
        ...
        private sealed class Vehicle
        {
            ...
            private readonly ClinicParkingPath entry,reverse,exit;
            internal Vehicle(GameObject root,int bay)
            {
                this.root=root;this.bay=bay;entry=ClinicParkingPath.Enter(bay);reverse=ClinicParkingPath.Reverse(bay);exit=ClinicParkingPath.Exit(bay);
            ...
            internal void Render(ClinicPatientState owner,double tick,bool available)
            {
                ...
                Vector3 position=ClinicParkingLayout.BayCenter(bay),direction=ClinicParkingLayout.BayFacing(bay);float distance=0,steer=0;
                ...
                if(owner.Phase==ClinicPatientPhase.WaitingToPark)position=new Vector3(-32-bay*3,-.11f,-8.2f);
