...
Patients and staff have deterministic appearances, with variations in clothing, skin tone, hair, accessories and proportions. Pooled actors must reset their appearance when reused. Seated poses and treatment positions remain aligned with furniture across body sizes. Walking cycles follow actual travel, including queue advancement. Reduced motion retains essential walking while suppressing decorative gestures; freezing a walking rig while translating it is not an acceptable reduced-motion substitute.

Cars use the streets and parking bays; pedestrians use pavements and crossings. Parking has distinct entrance and exit gates, a driving aisle and marked passenger routes. Unavailable bays contain staged paving materials, survey markings and planters; an upgrade replaces only its newly opened pair of staging areas. Ambient traffic is bounded and does not create clinic income. Reduced motion suppresses cosmetic traffic and construction motion while preserving essential simulation feedback.

Wall runs meet at their corners and door jambs. The clinic keeps low front and internal cutaway walls so management remains visible, with continuous skirting and caps. Reception, care, waiting and refreshment routes pass through framed sliding doors. Exterior queue rows enter through the clinic doorway rather than cutting across its front wall. Opposing visitors use separate passing lanes through the main corridor and shared doorways, while workstation and seat interaction points remain fixed.
...
Eligible arriving patients reserve a free parking bay before their car enters. A car drives from the street through the entrance and aisle into that bay; only then does its patient emerge and walk to reception. After care, the patient returns to the car before it reverses and drives through the exit. The reservation remains held until the car clears the exit. Shared maneuvers wait for other vehicles and parking passengers to clear. Saved phases and deadlines restore the same progress after relaunch. The parking fee is five coins per car-park level, included in the patient's fixed check-in quote. Bay occupancy never produces revenue on its own.
...
Street crossing eligibility and visible traffic share an operational clock. A saved `PausedTrafficTicks` offset excludes absence beyond the eight-hour earnings cap, so returning after a longer absence cannot put a departing car into a visually active pedestrian crossing. Older saves default this offset to zero. Construction still follows the full elapsed clock.
